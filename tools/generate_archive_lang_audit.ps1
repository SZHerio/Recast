param(
    ...
    [string]$CsvPath = (Join-Path $PSScriptRoot '..\tmp\codex_ru_audit_20260808\archive_lang_findings.csv'),
    [string]$ExclusionsCsvPath = (Join-Path $PSScriptRoot '..\tmp\codex_ru_audit_20260808\archive_lang_exclusions.csv'),
    [string]$SummaryPath = (Join-Path $PSScriptRoot '..\tmp\codex_ru_audit_20260808\archive_lang_summary.json')
)
...
function Test-IsModName {
    ...
    # Название вкладки или окна не обязано быть названием мода: например,
    # "Car Parts" и "NTM Ores and Blocks" требуют перевода. Защищаем только
    # явные ключи имени мода и значения, совпадающие с displayName из metadata.
    $Key -match '(?i)(^|\.)(modname|mod_name)(\.|$)' -or
...
$sortedExclusions = @(
    $exclusions | Sort-Object classification, namespace, key, source
)
...
$summary = [pscustomobject][ordered]@{
    ...
    outputs = [ordered]@{
        ...
        exclusions_csv = (Get-RelativePath $ExclusionsCsvPath)
    ...
    translation_policy = [ordered]@{
        ...
        protected_source_names = @('mod names', 'concrete weapon/ammunition/explosive names')
        weapon_descriptions_components_and_upgrades_are_not_protected = $true
    }
    inventory = [ordered]@{
...
$exclusionCsvLines = @($sortedExclusions | ConvertTo-Csv -NoTypeInformation -Delimiter ',')
$exclusionCsvText = [string]::Join("`r`n", $exclusionCsvLines) + "`r`n"
[System.IO.File]::WriteAllText($ExclusionsCsvPath, $exclusionCsvText, $utf8Bom)
