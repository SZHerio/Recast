...
// его применения живут в 07_strategic_programme.js.
//
...
function ifRemoveFolder(event, folder, expected) {
  ...
  let hbmPattern = null
  try {
    // event.remove в KubeJS 6 больше не возвращает число снятых рецептов,
    // поэтому счёт ведётся отдельным проходом до снятия. Без него проверка
    // сравнивала undefined с ожидаемым и ругалась на исправный слой.
    hbmPattern = { id: new RegExp(`^hbm_ntm_rebirth:${folder}/`) }
    removed = 0
    event.forEachRecipe(hbmPattern, () => {
      removed = removed + 1
    })
    event.remove(hbmPattern)
  } catch (error) {
    console.error(
...
ServerEvents.recipes((event) => {
  ...
  }).id('industrial_frontier:hbm/anvil_iron')
  ...
  }).id('industrial_frontier:hbm/anvil_lead')
  ...
  }).id('industrial_frontier:hbm/assembly_machine')
