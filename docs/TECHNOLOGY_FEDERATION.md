...
flowchart TD
    ...
    C1["Городская программа P1"] -. открывается .-> C2["P2"]
    C2 --> C3["P3"] --> C4["P4"] --> C5["P5"] --> C6["P6"] --> C7["P7"] --> C8["P8"] --> C9["P9"]
    P1 -. снабжает .-> C1
    P4 -. снабжает .-> C4
    P9 -. снабжает .-> C9
```

Городские узлы образуют собственную необязательную магистраль. Они используют
продукцию соответствующих технических эпох, но не являются предками их
приёмочных квестов и не повышают `tech_epoch`.

Внутри проекта ветви разных модов расходятся и снова сходятся на работающем объекте. Например, глава «Атомная индустрия» включает химию очистки, изготовление топлива, реакторную физику, тепловой контур, турбину, защиту, склад ОЯТ и переработку отходов. Подключение городской подстанции остаётся отдельным необязательным применением этого контура.
