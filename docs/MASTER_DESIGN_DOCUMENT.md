...
| Поле | Значение |
|---|---|
| Статус | M0–M9 собраны на файловом уровне: 112 `DONE`, 3 `OWNER`, 6 `DEFERRED` в [`registries/milestone_status.json`](registries/milestone_status.json); M9 ожидает визуальный пользовательский gate |
| Дата последнего полного аудита | 9 августа 2026 года |
...
Текущий визуальный срез зафиксирован в
[`M9_IMPLEMENTATION_REPORT.md`](history/M9_IMPLEMENTATION_REPORT.md); статус билда и
порядок ручной проверки — в [`VERSION_LOG.md`](VERSION_LOG.md) и
[`M9_BUILD_TEST_PROTOCOL.md`](history/M9_BUILD_TEST_PROTOCOL.md). Исторические отчёты
M1–M8.5 сохранены рядом и не заменяют актуальный instance-audit.

### Текущий контрольный срез `IF-M9-0001`

- 133 активных JAR; неизвестных CurseForge-источников `0`, зарегистрированное
  исключение `1`, расхождений source-hash `0`, обязательных dependency-failure
  `0`.
- Quest Source v2, staging и активная runtime-книга синхронизированы: 27 глав,
  501 квест, 822 доказательства и 668 причинных рёбер. Перед продвижением создана
  резервная копия `backups/questbook_runtime_before_v2_20260809-083807`, а
  `data.snbt` сохранён без изменения. Статический итог — `PASS_RUNTIME_SYNC`;
  запуск игры и визуально-игровая приёмка остаются пользовательским gate.
  Авторские словари Recast среза IF-M9 имеют 1428 русских и 1428 английских
  ключей; в его compiled-SNBT нет кириллических литералов.
- M9 подключает восемь layout FancyMenu/Drippy, тему FTB Quests, десять иконок
  эпох, 22 используемых знака процессов, 136 рабочих ассетов и шесть сохранённых
  исходных кадров с provenance и SHA-256.
- По всему milestone-реестру: 112 `DONE`, 3 пользовательских `OWNER` и 6
  сознательных `DEFERRED`. Minecraft при подготовке среза не запускался.
- Полная русификация **не закрыта**: после overlays остаются 7 345 ключей
  сторонних модов, одна пустая строка, 25 placeholder-mismatch и 104 страницы
  NuclearCraft GuideME. Это обязательный release-gate M10, а не завершённый
  пункт M9.
- Пять собственных машин и тиры планетных измерений остаются отключены по
  причинам, перечисленным в [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md).

Полный статический снимок лежит в
[`registries/generated/IF-M9-0001/`](registries/generated/IF-M9-0001/).

### Исторический контрольный срез `IF-M4-0001`
...
### 2.6. Происхождение модов

> **Решение владельца от 10 августа 2026 года: правило «моды только с
> CurseForge» отменено.**
>
> Оно вводилось ради воспроизводимости, но на практике мешало: нужные моды
> публикуются и на других площадках, а исключения приходилось заводить вручную.
> CC:Tweaked, из-за которого выпуск считался заблокированным, перестаёт быть
> блокером.
>
> Что остаётся в силе: у каждого JAR фиксируются имя, версия, источник и
> локальный SHA-256, а состав сборки сверяется с реестром после любого
> изменения модлиста. Проверяемость важна, а площадка — нет.

### 2.6.1. Прежняя редакция правила (историческая)
...
Текущий статус на 2026-08-09 отличается от этого исторического снимка: Just Enough Resources выведен из активного набора в восстановимый карантин из-за несовместимости его JEI-плагина с динамическим биомом NuclearCraft. Поиск рецептов, чтение блока и обучение геологии обеспечивают JEI, Jade и сама квестовая книга.
...
| P4 — Промышленная химия | Управлять реактивами, жидкостями, газами и возвратными потоками | Серная, азотная и хлорная химия, удобрения, водоподготовка и общий промышленный заказ | Химические домены, орошение и нефтехимия; муниципальные службы — необязательный потребитель |
| P5 — Региональная фабрика и логистика | Масштабировать фабрику и связать промышленные районы | Массовая переработка, железные дороги/магистрали, автоматический склад | GT MV/HV, Create-транспорт, AE2; городские поставки и строительные материалы — необязательная интеграция |
| P6 — Атомная индустрия | Освоить топливный цикл и безопасную базовую генерацию | Реакторное топливо, промышленная АЭС, замкнутый тепловой контур, хранение и переработка ОЯТ | NuclearCraft, Dea's Fission при уникальной роли, химия; городская энергосеть — необязательная нагрузка |
...
| P8 — Орбитально-планетарная сеть | Сделать внеземные базы частью общей экономики | Постоянный межпланетный грузопоток, жизнеспособная внеземная база, синтез и высокоэнергетические процессы | Планеты, AE2/CC, NuclearCraft, HBM и редкие материалы; внеземной городской контракт — необязателен |
| P9 — Межпланетная цивилизация | Завершить не предмет, а устойчивую распределённую систему | Финальный мегапроект с непрерывным снабжением, проверкой устойчивости и замкнутыми циклами | Все технические домены, мегаструктуры и долгосрочные испытания; городская сеть — необязательный потребитель |

Городская программа образует отдельную необязательную магистраль P1–P9. Она
открывается от технических вех, создаёт осмысленный спрос на их продукцию и
имеет собственную последовательность, но ни один её узел не входит в
обязательный результат эпохи или в условия `commissioning`.
...
Контракт — логичный sink массовых ресурсов и доказательство производства внутри необязательной городской кампании. Ни один городской контракт не открывает техническую эпоху и не входит в её `commissioning`; он не должен быть скрытым налогом.
...
- как пользоваться JEI, Jade, квестовыми ссылками и картой геологии;
- как искать GT-жилы;
...
Статический слой критерия выхода реализован в `IF-M2-0001`: схемы и реестры проходят локальный validator/lint. Runtime-часть остаётся пользовательским gate: полный registry/JEI-census GTCEu, фактическая генерация, удалённые обходы, коэффициент энергетического моста и визуальная проверка Академии подтверждаются по [`M2_BUILD_TEST_PROTOCOL.md`](history/M2_BUILD_TEST_PROTOCOL.md). До этого M2 имеет статус `STATIC_COMPLETE / USER_GATE_PENDING`, а не `ACCEPTED`.
...
Критерий выхода: химия одновременно питает минимум три технические системы; каждый обязательный процесс можно собрать и починить по русской книге. Город создаёт полезный, прозрачный и уже автоматизируемый массовый спрос только внутри добровольно начатой кампании.

Исторический статический слой `IF-M4-0001` содержал главу P4 из 13 квестов от разделения трёх классов воды до первого муниципального контракта. В Quest Source v2 эта топология заменена: техническая химия закрывается собственным промышленным заказом, а установленный стек MineColonies развивается в отдельной необязательной кампании P1–P9. Совпадение рецептов и поведения городских механизмов в игре остаётся пользовательским gate.
...
- подключение городской сети к АЭС как необязательный междоменный мегапроект;
...
Статический слой критерия выхода реализован в `IF-M6-0001`: все 11 пунктов `DONE`. Вход в домен перенесён за сталь и электронику HV — штатно первая машина NuclearCraft стоила свинца и поршня, то есть атомная промышленность открывалась в первую эпоху. Замок стоит на двух машинах входа, поэтому 4331 рецепт мода не тронут, а его реакторная физика не подменена имитацией. Обязательная связь с прежними доменами обеспечена: графит приходит из коксовой линии P2, вода и химия — из P4, а полезная энергия подтверждается собственным техническим контуром. Подача энергии в городскую сеть P5 остаётся необязательным применением. Фактическая проходимость и безопасность остаются пользовательским gate.
...
- самостоятельный технический космодром и его необязательная связь с городским районом;
- первый запуск;
...
- обязательные связи HBM с NuclearCraft, химией и космосом; город подключается только внутри необязательной кампании;
...
Критерий выхода: HBM имеет самостоятельную стратегическую роль без второго радиационного счётчика и непреднамеренных обходов; космос требует результаты всех обязательных технических систем и после открытия возвращает ценность в земную прогрессию. Городская программа может расширять космодром, но не является условием запуска.
...
Статический слой критерия выхода реализован в `IF-M8-0001`: из 11 пунктов 10 `DONE` и один `DEFERRED` решением владельца о единой волне перевода. Финалов два, и это прямой ответ на критерий: сфера Дайсона проверяет масштаб — её нельзя построить в одиночку и нельзя бросить, брошенная разрушается; колония на Проксиме b проверяет автономность — она обязана жить, когда Земля перестанет присылать корабли. Ни один из двух не сводится к дорогому предмету, оба имеют условие отказа, и ни один не закрывает другой.

Три решения владельца от 4 августа 2026 определили форму этапа. Синтез разведён на две роли вместо трёх лестниц к одному результату: реактор GregTech остаётся станком редких материалов, лазерная установка HBM становится главной электростанцией финала, ядро синтеза NuclearCraft снимается как третья копия. Планетарная сеть включает и грузопоток, и орбитальные службы: планеты работают цехами одного завода, а орбита даёт разведку, навигацию и передачу энергии лучом. Семь ускорительных установок, отложенных в шестой эпохе, возвращены здесь — отложенное не должно превращаться в забытое.

Термоядерная ветка HBM воротами не закрывается, и это установлено разбором, а не предположением: её плазменная печь требует сверхпроводящей проволоки BSCCO, турбина — проволоки из шрабидия и квантовых схем. Лестница уже внутри мода — тот же вывод, что по GCYR в M7. Фактическая проходимость и производительность остаются пользовательским gate.

### M8.5 — Собственная техника, живой мир и модуль фракций

Этап заведён 4 августа 2026 года и вклинивается между M8 и M9. Полное описание с обоснованием каждого пункта — в [`CONTENT_AND_MACHINERY_PLAN.md`](CONTENT_AND_MACHINERY_PLAN.md).

Работы:

- собственные мультиблоки сборки: водоочистная станция, установка дезактивации, пост радиационного контроля;
- собственные мультиблоки сборки: стенд сборки ступени и станция обслуживания роя;
- марафоны выбора: энергия, логистика, атом, космос и финал;
- список точных файлов CurseForge для волны структур мира;
- список точных файлов CurseForge для волны техники и хранения;
- модуль фракций: ядро состояния, эпоха, репутация и договоры;
- модуль фракций: базы B0–B6 в мире на готовых шаблонах структур;
- модуль фракций: жизненный цикл операций с предупреждением и восстановлением;
- модуль фракций: NPC, диалоги и переговоры;
- модуль фракций: развитие фракций от прогресса игрока;
- полная русская локализация собственного модуля и заимствованного контента.

Критерий выхода: фракции существуют в мире как игровая механика, а не как описание; каждое обещание сборки о собственном процессе подкреплено машиной, которую можно построить.

Причина появления этапа названа прямо. Прямая проверка 4 августа 2026 года показала, что работающей фракционной механики в сборке нет: есть спецификация, шесть схем, определения баз, фракций, операций и запретов — и есть команды, печатающие описания. Ни одной строки, создающей существо, ставящей структуру или наносящей урон, не написано. План это предусматривал: фракциями владеет собственный модуль, и до его появления фракционный срез не выпускается. Модуля не было, и M8.5 существует, чтобы он появился.

Этап закрыт 4 августа 2026 года. Фракции стали механикой: у них есть места на карте, живые представители, разговор с настоящими условиями, репутация, которая меняется только по названной причине, договоры со своей ценой и операции со временем, предупреждением и окном переговоров. Собственный JAR при этом не понадобился — разбор показал, что движок фракций уже стоит в сборке в составе Easy NPC, и сборке оставалось дать ему содержание.

Отдельно из этого этапа вышла находка, к фракциям отношения не имеющая. Тридцать одна команда сборки объявлялась способом, при котором в игре она вызывается только длинной служебной формой, тогда как книга заданий и все протоколы проверки обещают короткое имя вида `/if_operations`. Шестьдесят таких обещаний не сработали бы при первом же запуске. Короткие имена теперь зарегистрированы по-настоящему, а линтер следит, чтобы список не разошёлся с набором обработчиков.
...
Статический срез `IF-M9-0001` собран 8 августа 2026 года. FancyMenu 3.9.9 и
Drippy Loading Screen 3.1.5 связаны восемью layout; Paxi доставляет тему и 136
зарегистрированных рабочих ассетов, а provenance отдельно фиксирует шесть
исходных художественных кадров; книга использует десять иконок эпох и все 22 знака
производственных путей. Русский и английский интерфейс разделены lang-key,
кириллических литералов в compiled-квестах нет. Файловая часть прошла
[`validate_m9_visuals.ps1`](../tools/validate_m9_visuals.ps1); критерий выхода
остаётся открытым до визуального прогона владельца по
[`M9_BUILD_TEST_PROTOCOL.md`](history/M9_BUILD_TEST_PROTOCOL.md). Подробный состав — в
[`M9_IMPLEMENTATION_REPORT.md`](history/M9_IMPLEMENTATION_REPORT.md).
...
Статический срез `IF-M10-0001` собран 8 августа 2026 года без запуска
Minecraft/Forge. Реализованы машинный аудит прогрессии и
grind-budget P0–P9, четыре release-профиля, manifest/source/hash preflight,
лицензионный инвентарь 133 JAR, миграционные заметки, полный Known Issues,
статическая редакторская приёмка и единый пользовательский протокол времени,
раздражения, performance, multiplayer, оружия, NPC и операций.

После волны `IF-RU-0001` построчный статус M10: 14 пунктов — 6 `DONE`, 5
`OWNER`, 2 `PARTIAL`, 0 `NOT_DONE`, 1 `DEFERRED`. Полная русская локализация
слита и статически проверена, включая внутриигровые руководства и терминал
CC:Tweaked. Этап и S-tier DoD всё ещё **не закрыты**: открыты фактические
пользовательские прогоны, non-CurseForge CC:Tweaked 1.120.0 и непрерывная
вычитка обязательной ветки квестбука в игре. Авторитетный ledger —
[`registries/release_readiness.json`](registries/release_readiness.json),
реализация — [`M10_IMPLEMENTATION_REPORT.md`](history/M10_IMPLEMENTATION_REPORT.md),
пользовательский gate —
[`M10_BUILD_TEST_PROTOCOL.md`](history/M10_BUILD_TEST_PROTOCOL.md).
...
- химия и космос влияют на главный путь, а город связывает их продукцию в необязательной интеграционной кампании;
...
9. Целевая длина первого полного прохождения; доля городских контрактов, обязательных для `commissioning` P0–P9, уже зафиксирована как 0%.
...
12. После первой устойчивой прогрессии перейти к промышленной химии P4 и параллельному расширению необязательной городской кампании, затем к региональным операциям P5, NuclearCraft P6 и экспериментальному HBM/space-gate P7.
...
| 2026-08-04 | Отчёты пишутся для игрока, а не для инженера | Владелец хочет получить готовую сборку, а не консультацию по внутреннему устройству; точность и числа остаются обязательными, но объясняются через то, что меняется в игре — правило записано в REPORTING_RULES, раздел 4.1 |
| 2026-08-04 | Термоядерный синтез разведён на две роли: цех материалов и электростанция | Три мода умеют синтез, и это три прохождения одного и того же. Реактор GregTech остаётся станком редких материалов, лазерная установка HBM становится главной электростанцией финала, синтез NuclearCraft снимается как третья копия |
| 2026-08-04 | У сборки два финала: сфера Дайсона и колония на чужой звезде | Один проверяет энергию и масштаб, другой — автономность и выживание; прохождение перестаёт быть однорельсовым, а «устойчивая цивилизация» из критерия выхода получает два разных доказательства |
| 2026-08-04 | Планетарная сеть включает и грузопоток, и орбитальные службы | Планеты работают как цеха одного завода и обмениваются продукцией без участия игрока, а орбита даёт услуги сверху: поиск руды, навигацию и передачу энергии лучом |
| 2026-08-04 | Модуль фракций пишется локальным модом сборки, без публикации на CurseForge | Правило «сторонние моды только с CurseForge» на него не распространяется: это часть сборки, как датапак и скрипты. modid `industrial_frontier` — все идентификаторы, написанные за восемь волн, становятся его настоящими ID без единой правки |
| 2026-08-04 | Сборка получает собственные мультиблоки GregTech вместо переписывания чужих | За восемь волн не создано ни одной своей машины; пять обещаний игроку — водоочистка, дезактивация, радиационный контроль, сборка ступени и обслуживание роя — держались на тексте |
| 2026-08-04 | Заведён этап M8.5 между M8 и M9 | Работа по контенту, собственной технике и модулю фракций не входила ни в один существующий этап; полуцелый номер не ломает нумерацию плана |
| 2026-08-04 | Валюта, магазин и квесты, забирающие ресурсы, не вводятся | Решение владельца при разборе HardTech: правило «квест не поглощает произведённое» остаётся |
| 2026-08-04 | Книга заданий открывается постепенно, а не лежит перед игроком целиком | Восемнадцать глав и сто семьдесят восемь заданий с первой минуты — это не руководство, а стена: игрок видел ядерную главу и оба финала, ещё не добыв камня. Главы связаны в цепочку, всегда открыты только вход и справка. Внутри главы видно всё: игрок должен понимать объём эпохи и планировать |
| 2026-08-04 | У заданий нет наград | Решение владельца. Книга объясняет и ведёт, а не платит за прохождение; 155 блоков наград сняты, генератор их больше не создаёт |
| 2026-08-04 | Разметка книги уплотнена втрое | Иконки 2.0 при шаге 3.0 оставляли между заданиями пустоту, и глава из четырнадцати заданий растягивалась на двадцать четыре клетки. Разбор чужой книги на девятьсот заданий дал рабочие пропорции; теперь иконка 1.0 при шаге 1.5, а раскладка считается по графу связей |
| 2026-08-04 | Шесть глав дописаны по итогам сверки покрытия | Сверка показала, что о хранении, обороне, поселении, пневматике, фотолитографии, линии сборки компонентов, нефти и работающих фракциях книга не говорила ничего. Тридцать пять заданий закрывают эти пробелы |
| 2026-08-04 | Модуль фракций написан средствами сборки, собственный JAR не понадобился | Решение о локальном моде принималось из того, что фракционной механики нет и взять её неоткуда. Разбор показал обратное: Easy NPC 7.4.1 везёт готовый движок — сохраняемые фракции с матрицей вражды, цель «атаковать враждебные фракции», разговоры с кнопками, условия кнопок по табло и действия-команды. Правила и состояние остались на скриптах сборки, тела и разговоры отданы Easy NPC, отряды — TaCZ. Ничего, что нельзя проверить статически, в сборке не появилось |
| 2026-08-04 | Команды сборки получили собственные имена | Тридцать одна команда объявлялась через customCommand и вызывалась в игре только длинной формой «/kubejs custom_command …», тогда как книга и протоколы обещали «/if_operations». Шестьдесят обещаний не сработали бы при первом же запуске. Короткие имена зарегистрированы по-настоящему, а линтер сверяет список моста с набором обработчиков |
| 2026-08-04 | Пять уровней выше UHV делаются целиком, но в версии 2.0 | Настройка `highTierContent` не переключатель: включённые ею предметы рецептов не имеют по заявлению самого мода, GT-- их не наполняет, около 460 позиций пришлось бы обеспечить цепочками самостоятельно. Приоритет отдан доведению сборки до 1.0; список отложенного — `ROADMAP_2_0.md` |
