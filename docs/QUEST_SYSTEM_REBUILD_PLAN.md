...
Статус: `AUDIT_COMPLETE / IMPLEMENTATION_IN_PROGRESS`  
Дата среза: 8 августа 2026 года  
Область: полная квестовая книга, P0–P9, все 133 активных JAR  
...
Новая книга будет построена как граф инженерных способностей. У каждой эпохи P0–P9 появится короткая центральная ось и ровно один узел ввода эпохи в эксплуатацию — `commissioning`. К нему сходятся все действительно обязательные производственные ветви этой эпохи; только он повышает командный `industrial_frontier:tech_epoch` и открывает следующую эпоху. Подробные уроки по геологии, машинам, химии, городу, логистике, атомной отрасли, вооружению и космосу живут в предметных главах и подключаются к оси видимыми ссылками. Функционально равноценные источники принимаются через явно проверенные `ONE_OF`-наборы или pack-owned теги, а построить, запустить, посетить, перевезти и произвести больше нельзя будет подтверждать обычной галочкой.
...
- P4 искусственно ставит серную, азотную и хлорную химии в одну линию. Это независимые сырьевые ветви, которые должны сходиться у общего городского потребителя.
...
| Settlement | Короткая отдельная ветка без городского join | Сквозная городская программа P1–P9 с функциональными контрактами |
...
5. A — утверждённый сюжетный/городской контракт, который создаёт измеримый спрос на B;
...
| Группа | Назначение |
|---|---|
| `00 — Начать здесь` | Интерфейс, JEI/JER/Jade, команда, карта, правила книги, помощь |
...
Добро пожаловать
  ...
  ├─ Jade/JER: что показывает блок и где искать ресурс
...
| P4 — Химический город | Независимые sulfur/nitrogen/chlorine ветви; вода/стоки; удобрения/орошение; нефтяная разведка и перегонка; серийная еда; коммунальный спрос | Городской контракт одновременно получает воду, рацион, удобрение/урожай и продукт химической линии; отход имеет назначение | MTR, Ultimate Car, ферментация, дополнительные городские сервисы |
| P5 — Региональная фабрика | Массовая переработка; грузовой rail/water; региональные буферы; AE2 storage→channels→patterns→autocraft→recovery; городская логистика | Производственный заказ проходит от удалённого сырья через массовый процесс и транспорт в автоматизированный склад без ручной перекладки каждого цикла | Advanced Peripherals, ProjectRed fabrication, торговые специализации, Multi Builder Tool после ручного multiblock |
| P6 — Атомная индустрия | Ядерное сырьё; purification/conversion/enrichment; fuel fabrication; dosimetry; reactor; heat loop; turbine; spent fuel; reprocessing/storage | Гражданская АЭС выдаёт полезную энергию в городскую сеть, а отработанное топливо попадает в контролируемый маршрут | Изотопные продукты, альтернативные NuclearCraft-конфигурации, мастерство эффективности |
...
- city contract сделать join-узлом спроса;
...
Выход: химический город работает как сеть потребителей и возвратов, а не как три кислоты в ряд.
...
- связать MineColonies, Lightman's Currency и региональные контракты;
...
- связать графит P2, воду/химию P4 и городскую сеть P5;
...
| `14_p4_chemical_city` | Кислоты искусственно последовательны; exact water/fertiliser; disabled treatment обещан как доступный | Три параллельные химические ветви плюс вода/городские consumers; disabled-механику убрать из обязательного текста |
...
| `22_settlement` | MineColonies сведён к пяти галочкам, снабжение не соединено с промышленностью | Сквозная городская программа с функциональными комнатами, складами, профессиями, энергией и contracts |
...
### 18.5. Игровые вспомогательные системы — 10 JAR

| Мод | Класс | Покрытие |
|---|---|---|
| Just Enough Items | ФС | Onboarding: recipe/use/category/tag/bookmark/@mod |
| Just Enough Resources | ФС | P0 geology: distribution/loot с предупреждением о GT worldgen authority |
